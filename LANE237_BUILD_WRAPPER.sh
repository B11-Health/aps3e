...
SELF="$WT/LANE237_BUILD_WRAPPER.sh"
LAUNCHER="$WT/LANE237_CXX_AUDIT_LAUNCHER.sh"
...
HEAVY_LABEL="lane237-bink-putllc-build-v2"
...
LAUNCHER_DESIGN_SHA="a6af86b92f1b737a66c6ff0dbee59bf3e39676d8a10dd281e3dc508dac3a398a"
...
BUILD="$HOME_ROOT/.cache/gd-lane237-bink-putllc-build-v2"
...
fail() {
    local rc="$1"; shift
    printf 'LANE237_FAIL stage=%s rc=%s %s\n' "$STAGE" "$rc" "$*" >&2
    exit "$rc"
}
sha_file() {
    local out
...
[[ "$APPROVED_COMMIT" =~ ^[0-9a-f]{40}$ ]] || fail 72 'LANE237_APPROVED_COMMIT missing/invalid'
[[ "$APPROVED_WRAPPER_SHA" =~ ^[0-9a-f]{64}$ ]] || fail 73 'LANE237_APPROVED_WRAPPER_SHA256 missing/invalid'
[[ "$APPROVED_LAUNCHER_SHA" =~ ^[0-9a-f]{64}$ ]] || fail 74 'LANE237_APPROVED_LAUNCHER_SHA256 missing/invalid'
[[ "$APPROVED_LAUNCHER_SHA" == "$LAUNCHER_DESIGN_SHA" ]] || fail 75 'approved launcher hash != reviewed Lane237 launcher design'
...
head=$(git -C "$WT" rev-parse HEAD) || fail 83 'Lane237 rev-parse failed'
[[ "$head" == "$APPROVED_COMMIT" ]] || fail 84 "Lane237 HEAD != approved commit: $head"
[[ "$(git -C "$WT" symbolic-ref --short HEAD)" == "team/lane237-bink-putllc-build-v2" ]] || fail 85 'Lane237 branch mismatch'
[[ "$(git -C "$WT" show -s --format=%P HEAD)" == "$LANE230" ]] || fail 86 'Lane237 approved HEAD must be exactly one commit atop Lane230'
git -C "$WT" merge-base --is-ancestor "$LANE220" "$head" || fail 87 'Lane220 is not ancestor of Lane237'
[[ -z "$(git -C "$WT" status --porcelain=v1 -uall)" ]] || fail 88 'Lane237 worktree must be clean'
[[ "$(git -C "$WT" hash-object "$SELF")" == "$(git -C "$WT" rev-parse "$APPROVED_COMMIT:LANE237_BUILD_WRAPPER.sh")" ]] || fail 89 'wrapper differs from approved Git blob'
[[ "$(git -C "$WT" hash-object "$LAUNCHER")" == "$(git -C "$WT" rev-parse "$APPROVED_COMMIT:LANE237_CXX_AUDIT_LAUNCHER.sh")" ]] || fail 90 'launcher differs from approved Git blob'

git -C "$WT" diff --quiet "$LANE230" "$head" -- app/src/main/cpp || fail 91 'Lane237 altered Lane230 candidate source tree'
...
for t in cmake ninja ccache git sha256sum readelf file strings flock readlink awk grep; do command -v "$t" >/dev/null 2>&1 || fail 123 "missing tool $t"; done
...
: > "$TRACE"
chmod 600 "$TRACE"
export LANE237_CXX_TRACE="$TRACE"
export LANE237_APPROVED_COMMIT="$APPROVED_COMMIT"
export LANE237_APPROVED_WRAPPER_SHA256="$APPROVED_WRAPPER_SHA"
export LANE237_APPROVED_LAUNCHER_SHA256="$APPROVED_LAUNCHER_SHA"
printf 'approved_commit=%s\nwrapper_sha256=%s\nlauncher_sha256=%s\ncanon_head=%s\nndk=%s\ntoolchain_sha256=%s\nsysroot=%s\nfree_kib=%s\n' \
    "$APPROVED_COMMIT" "$wrapper_sha" "$launcher_sha" "$CANON_HEAD" "$NDK" "$TOOLCHAIN_SHA" "$SYSROOT" "$FREE" > "$RUN/preflight.txt"
...
grep -Fq "CMAKE_HOME_DIRECTORY:INTERNAL=$SRC" "$CACHE" || fail 132 'configure did not consume Lane237 isolated source tree'
...
STAGE=build
env -u CPATH -u C_INCLUDE_PATH -u CPLUS_INCLUDE_PATH -u OBJC_INCLUDE_PATH -u LIBRARY_PATH -u CMAKE_PREFIX_PATH -u CMAKE_LIBRARY_PATH -u CMAKE_INCLUDE_PATH \
    cmake --build "$BUILD" --target emu --parallel 1 >"$RUN/build.log" 2>&1 || fail 140 'build failed'
