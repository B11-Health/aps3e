...
SELF_EXPECTED="$WORKTREE/LANE198_OVERLAY_CXX_LAUNCHER.sh"
WRAPPER_EXPECTED="$WORKTREE/LANE198_BUILD_WRAPPER.sh"
...
fail() {
    local rc="$1"
    shift
    printf 'LANE198_OVERLAY_FAIL rc=%s %s\n' "$rc" "$*" >&2
    exit "$rc"
}
...
[[ "$APPROVED_COMMIT" =~ ^[0-9a-f]{40}$ ]] || fail 70 "LANE198_APPROVED_COMMIT must be an exact 40-hex commit"
[[ "$APPROVED_WRAPPER_SHA" =~ ^[0-9a-f]{64}$ ]] || fail 71 "LANE198_APPROVED_WRAPPER_SHA256 must be an exact 64-hex SHA256"
[[ "$APPROVED_LAUNCHER_SHA" =~ ^[0-9a-f]{64}$ ]] || fail 72 "LANE198_APPROVED_LAUNCHER_SHA256 must be an exact 64-hex SHA256"
[[ -n "$TRACE" ]] || fail 73 "LANE198_TRACE_PATH is required"

TRACE_PREFIX="$HOME_ROOT/.cache/gd-lane198-lane187-canary-libe-${APPROVED_WRAPPER_SHA}-"
...
if (( sub_count == 1 )); then
    ...
    if ! lane198_head=$("$GIT" -C "$WORKTREE" rev-parse HEAD); then
        fail 93 "Lane198 rev-parse failed"
    fi
    [[ "$lane198_head" == "$APPROVED_COMMIT" ]] || fail 94 "Lane198 approved commit mismatch: $lane198_head"
    clean_status_or_fail "$WORKTREE" "Lane198"
