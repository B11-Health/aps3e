...
SELF="$WT/LANE241_CXX_AUDIT_LAUNCHER.sh"
WRAPPER="$WT/LANE241_BUILD_WRAPPER.sh"
CANON="$HOME_ROOT/projects/android/gamedeck/mobile/android/vendor/aps3e-source"
TRACE="${LANE241_CXX_TRACE:-}"
BASE237="d787644dc8c1c5076047dd6b36c8bfff659a85db"
...
fail() {
    local rc="$1"; shift
    printf 'LANE241_CXX_AUDIT_FAIL rc=%s %s\n' "$rc" "$*" >&2
    exit "$rc"
}
...
[[ "$APPROVED_COMMIT" =~ ^[0-9a-f]{40}$ ]] || fail 70 'LANE241_APPROVED_COMMIT missing/invalid'
[[ "$APPROVED_WRAPPER_SHA" =~ ^[0-9a-f]{64}$ ]] || fail 71 'LANE241_APPROVED_WRAPPER_SHA256 missing/invalid'
[[ "$APPROVED_LAUNCHER_SHA" =~ ^[0-9a-f]{64}$ ]] || fail 72 'LANE241_APPROVED_LAUNCHER_SHA256 missing/invalid'
...
[[ "$("$GIT" -C "$WT" show -s --format=%P HEAD)" == "$BASE237" ]] || fail 78 'approved commit is not the single Lane237-child correction'
...
[[ "$("$GIT" -C "$WT" hash-object "$SELF")" == "$("$GIT" -C "$WT" rev-parse "$APPROVED_COMMIT:LANE241_CXX_AUDIT_LAUNCHER.sh")" ]] || fail 81 'launcher differs from approved Git blob'
[[ "$("$GIT" -C "$WT" hash-object "$WRAPPER")" == "$("$GIT" -C "$WT" rev-parse "$APPROVED_COMMIT:LANE241_BUILD_WRAPPER.sh")" ]] || fail 82 'wrapper differs from approved Git blob'
