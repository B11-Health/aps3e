...
fail() {
    local rc="$1"
    shift
    printf 'LANE202_OVERLAY_FAIL rc=%s %s\n' "$rc" "$*" >&2
    exit "$rc"
}
...
[[ "$APPROVED_COMMIT" =~ ^[0-9a-f]{40}$ ]] || fail 70 "LANE202_APPROVED_COMMIT must be an exact 40-hex commit"
[[ "$APPROVED_WRAPPER_SHA" =~ ^[0-9a-f]{64}$ ]] || fail 71 "LANE202_APPROVED_WRAPPER_SHA256 must be an exact 64-hex SHA256"
[[ "$APPROVED_LAUNCHER_SHA" =~ ^[0-9a-f]{64}$ ]] || fail 72 "LANE202_APPROVED_LAUNCHER_SHA256 must be an exact 64-hex SHA256"
[[ -n "$TRACE" ]] || fail 73 "LANE202_TRACE_PATH is required"

TRACE_PREFIX="$HOME_ROOT/.cache/gd-lane202-lane187-canary-libe-${APPROVED_WRAPPER_SHA}-"
...
if (( sub_count == 1 )); then
    ...
    if ! lane198_head=$("$GIT" -C "$WORKTREE" rev-parse HEAD); then
        fail 93 "Lane202 rev-parse failed"
    fi
    [[ "$lane198_head" == "$APPROVED_COMMIT" ]] || fail 94 "Lane202 approved commit mismatch: $lane198_head"
    clean_status_or_fail "$WORKTREE" "Lane202"
