	...
	namespace reports
	{
		...
		void ZCULL_control::read_report(::rsx::thread* ptimer, vm::addr_t sink, u32 type)
		{
			...
			else
			{
				...
				{
					// Immediate report writes are strongly ordered with prior deferred MM operations.
					rsx::mm_flush();
		...
		{
			// Reports are strongly ordered. Flush only deferred host-MM work that predates this report.
		...
		u32 ZCULL_control::copy_reports_to(u32 start, u32 range, u32 dest)
		{
			...
			for (auto& writer : m_pending_writes)
			{
				...
				if (!writer.forwarder && memory_range.overlaps(writer.sink))
				{
					...
					{
						bytes_to_write += sizeof(CellGcmReportData);
