...
void VKGSRender::update_draw_state()
{
		...
		rsx::method_registers.current_draw_clause.primitive <= rsx::primitive_type::line_strip)
	{
		...
		_vkCmdSetLineWidth(*m_current_command_buffer, actual_line_width);
	}
	...
	if (rsx::method_registers.blend_enabled())
	{
		...
		_vkCmdSetBlendConstants(*m_current_command_buffer, blend_colors.data());
	}
	...
	if (rsx::method_registers.stencil_test_enabled())
	{
		...
		_vkCmdSetStencilWriteMask(*m_current_command_buffer, face_flag, rsx::method_registers.stencil_mask());
		_vkCmdSetStencilCompareMask(*m_current_command_buffer, face_flag, rsx::method_registers.stencil_func_mask());
		_vkCmdSetStencilReference(*m_current_command_buffer, face_flag, rsx::method_registers.stencil_func_ref());

		if (two_sided_stencil)
		{
			_vkCmdSetStencilWriteMask(*m_current_command_buffer, VK_STENCIL_FACE_BACK_BIT, rsx::method_registers.back_stencil_mask());
			_vkCmdSetStencilCompareMask(*m_current_command_buffer, VK_STENCIL_FACE_BACK_BIT, rsx::method_registers.back_stencil_func_mask());
			_vkCmdSetStencilReference(*m_current_command_buffer, VK_STENCIL_FACE_BACK_BIT, rsx::method_registers.back_stencil_func_ref());
		}
	}
	...
	if (rsx::method_registers.poly_offset_fill_enabled())
	{
		...
		_vkCmdSetDepthBias(*m_current_command_buffer, polygon_offset_bias, 0.f, polygon_offset_scale);
	}
	else
	{
		// Zero bias value - disables depth bias
		_vkCmdSetDepthBias(*m_current_command_buffer, 0.f, 0.f, 0.f);
	}
	...
	if (m_device->get_depth_bounds_support())
	{
		...
		_vkCmdSetDepthBounds(*m_current_command_buffer, bounds_min, bounds_max);
	}
...
void VKGSRender::emit_geometry(u32 sub_index)
{
    ...
    const bool is_view=cfg_vertex_buffer_upload_mode_use_buffer_view();
    auto make_descriptor = [&](const vk::buffer_upload& data) -> VkDescriptorBufferUploadEX{
        return is_view? VkDescriptorBufferUploadEX(vk::VkDescriptorBufferViewEx(*data.ptr.view))
        : VkDescriptorBufferUploadEX(vk::VkDescriptorSubBufferEx(*data.ptr.sub));
    };

    VkDescriptorBufferUploadEX persistent_buffer = m_persistent_attribute_storage
            ? make_descriptor(*m_persistent_attribute_storage)
            : make_descriptor(*null_buffer_view);
    VkDescriptorBufferUploadEX volatile_buffer = m_volatile_attribute_storage
            ? make_descriptor(*m_volatile_attribute_storage)
            : make_descriptor(*null_buffer_view);
	...
	if (update_descriptors)
	{
        ...
        if (is_view)
        {
            m_program->bind_uniform(std::get<vk::VkDescriptorBufferViewEx>(persistent_buffer), vk::glsl::binding_set_index_vertex, m_vs_binding_table->vertex_buffers_location);
            m_program->bind_uniform(std::get<vk::VkDescriptorBufferViewEx>(volatile_buffer), vk::glsl::binding_set_index_vertex, m_vs_binding_table->vertex_buffers_location + 1);
        }
        else
        {
            m_program->bind_uniform(std::get<vk::VkDescriptorSubBufferEx>(persistent_buffer), vk::glsl::binding_set_index_vertex, m_vs_binding_table->vertex_buffers_location);
            m_program->bind_uniform(std::get<vk::VkDescriptorSubBufferEx>(volatile_buffer), vk::glsl::binding_set_index_vertex, m_vs_binding_table->vertex_buffers_location + 1);
        }
    }
	...
	if (!upload_info.index_info)
	{
		...
		{
			_vkCmdDraw(*m_current_command_buffer, upload_info.vertex_draw_count, draw_call.pass_count(), 0, 0);
		}
		else if (draw_call.is_single_draw())
		{
			_vkCmdDraw(*m_current_command_buffer, upload_info.vertex_draw_count, 1, 0, 0);
		}
		...
		else
		{
			...
			{
				_vkCmdDraw(*m_current_command_buffer, range.count, 1, vertex_offset, 0);
	...
	else
	{
		...
		_vkCmdBindIndexBuffer(*m_current_command_buffer, m_index_buffer_ring_info.heap->value, offset, index_type);

		if (draw_call.is_trivial_instanced_draw)
		{
			_vkCmdDrawIndexed(*m_current_command_buffer, upload_info.vertex_draw_count, draw_call.pass_count(), 0, 0, 0);
		}
		else if (rsx::method_registers.current_draw_clause.is_single_draw())
		{
			_vkCmdDrawIndexed(*m_current_command_buffer, upload_info.vertex_draw_count, 1, 0, 0, 0);
		}
		else if (m_device->get_multidraw_support())
		{
			const auto subranges = draw_call.get_subranges();
		...
		else
		{
			...
			{
				const auto count = get_index_count(draw_call.primitive, range.count);
				_vkCmdDrawIndexed(*m_current_command_buffer, count, 1, vertex_offset, 0, 0);
				vertex_offset += count;
			}
		}
	}
