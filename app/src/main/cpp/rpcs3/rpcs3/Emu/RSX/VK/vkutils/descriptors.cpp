...
namespace vk
{
	...
	namespace descriptors
	{
		...
		VkDescriptorSetLayout create_layout(const rsx::simple_array<VkDescriptorSetLayoutBinding>& bindings)
		{
			...
			CHECK_RESULT(_vkCreateDescriptorSetLayout(*g_render_device, &infos, nullptr, &result));
			return result;
		}
	}
	...
	void descriptor_pool::destroy()
	{
		...
		{
			_vkDestroyDescriptorPool((*m_owner), pool.handle, nullptr);
	...
	void descriptor_pool::reset(u32 subpool_id, VkDescriptorPoolResetFlags flags)
	{
		...
		CHECK_RESULT(_vkResetDescriptorPool(*m_owner, m_device_subpools[subpool_id].handle, flags));
	...
	VkDescriptorSet descriptor_pool::allocate(VkDescriptorSetLayout layout, VkBool32 use_cache)
	{
		...
		if (use_cache)
		{
			...
			CHECK_RESULT(_vkAllocateDescriptorSets(*m_owner, &alloc_info, m_descriptor_set_cache.data()));
		...
		else
		{
			...
			CHECK_RESULT(_vkAllocateDescriptorSets(*m_owner, &alloc_info, &new_descriptor_set));
		}
	...
	void descriptor_set::push(const VkDescriptorImageInfo* image_info, u32 count, VkDescriptorType type, u32 binding)
	{
		...
		_vkUpdateDescriptorSets(*g_render_device, 1, &writer, 0, nullptr);
	}
	...
	void descriptor_set::flush()
	{
		...
		_vkUpdateDescriptorSets(*g_render_device, num_writes, m_pending_writes.data(), num_copies, m_pending_copies.data());
